...
public class TaskBase : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        int nbrMonster = Random.Range(mapLevel+2, mapLevel+4);

        for (int i = 0; i < nbrMonster; i++)
        {
            int LevelMonster = Random.Range(mapLevel+1, mapLevel+2);

            string monsterName = "Enemy" + i;
            GameObject newMonster = null;
            newMonster = GameObject.Instantiate(MonsterPrefab, transform.position, Quaternion.identity);
